...
class PyppeteerMiddleware(object):
    ...
    async def _process_request(self, request, spider):
        ...
        # download pdf
        print('111, 打印 pdf_path')
        if pyppeteer_meta.get('pdf_path'):
            _pdf_path = pyppeteer_meta.get('pdf_path')
            print(222, _pdf_path)
            try:
                logger.debug('ready to download pdf. path is {}'.format(_pdf_path))
                assert isinstance(_pdf_path, str), 'path must be str'
                await page.pdf(path=_pdf_path, format='A4')
            except Exception as e:
                logger.error('error download pdf. error_msg: {}'.format(e))
                await page.close()
                await browser.close()
                return self._retry(request, 504, spider)
